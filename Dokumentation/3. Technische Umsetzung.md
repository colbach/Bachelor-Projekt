...
Obwohl die Toolbar allgemeine Komponente entwickelt wurde wird nur im Hauptfenster verwendet und ist dort eine der wichtigsten Komponenten. Die Toolbar wird durch die gleichnamige Klasse `Toolbar` repräsentiert und durch Die Klasse `ToobarDrafter` gezeichnet. Die Toolbar hat keine Position da sie automatisch davon ausgeht sich im oberen Bereich der View zu befinden. Sie unterstützt das Interface `MouseAndKeyboardListener` und kann damit an einem `InputManager` angemeldet werden.
