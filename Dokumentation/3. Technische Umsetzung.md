...
Eine Scrollbar wird durch die gleichnamige Klasse `Scrollbar` beschrieben und durch die Klasse `ScrollbarDrafter` gezeichnet. Eine Scrollbar besteht aus einer Scroll-Richtung (vertikal oder horizontal), einer Fläche (`Area`), einer Position (zwischen 0 und 1), und einer repräsentierten Höhe. Die Komponente implementiert das Interface `MouseAndKeyboardListener` lässt sich an einem `InputManager` anmelden. Sin dieser Komponente ist es dass sich die repräsentierte Fläche dynamisch ändern kann ohne das die position springt oder sonstige nicht benutzerfreundliche Effekte auftreten.  
