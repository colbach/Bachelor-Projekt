...
import main.startuptasks.StartupTask;
...
public class MainClass {
     ...
     */
    public static String pathForBuildinNodeClasses = null;//"./build/classes/";
    ...
    public static void main(String[] args) {
        ...
        StartupTaskBatchRunner tasks = new StartupTaskBatchRunner(new StartupTask[]{
            ...
            new StartupTask(5, "Klassenpfad ermitteln", true) {
                @Override
                public void run() {
                    HashSet<String> possibleMainClassLocations = null;
                    possibleMainClassLocations = new HashSet<String>();
                    
                    possibleMainClassLocations.add("." + File.separator);
                    
                    for (String classpath : System.getProperty("java.class.path").split(File.pathSeparator)) {
                        File f = new File(classpath);
                        if (f.exists()) {
                            String possibleMainClassLocation;
                            if (classpath.endsWith(".jar")) {
                                possibleMainClassLocation = f.getParent();
                            } else {
                                possibleMainClassLocation = classpath;
                            }
                            if (possibleMainClassLocation != null) {
                                if (!possibleMainClassLocation.endsWith(File.separator)) {
                                    possibleMainClassLocation += File.separator;
                                }
                                possibleMainClassLocations.add(possibleMainClassLocation);
                            }
                        }
                    }

                    for (String classpath : possibleMainClassLocations) {
                        File mainClassFile = new File(classpath + "main" + File.separator + "MainClass.class");
                        if (mainClassFile.exists()) {
                            pathForBuildinNodeClasses = classpath;
                            break;
                        }
                    }

                    if (pathForBuildinNodeClasses == null) {
                        System.err.println("MainClass nicht gefunden! An folgenden Stllen wurde gesucht:");
                        ArrayHelper.print(possibleMainClassLocations);
                    }
                }
            },
