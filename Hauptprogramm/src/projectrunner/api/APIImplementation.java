...
import projectrunner.SharedObjectSpace;
import projectrunner.TemporarySmartIdentifierContextImplementation;
...
public class APIImplementation implements API {
    ...
    public Object createInstanceFromSourceCode(SourceCode sourceCode) throws Exception, TerminatedException {
        if (stillActive()) {
            return CodeCompiler.getInstance().compileAndCreateNewInstance(sourceCode.getJavaCode());
        } else {
            throw new TerminatedException();
        }
    }
