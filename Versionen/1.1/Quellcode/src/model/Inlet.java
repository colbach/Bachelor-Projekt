...
public class Inlet extends InOutlet {
    ...
    public boolean canConnectToOutlet(Outlet outlet) {
        ...
        boolean canTypeConnectToType = CompatibilityTest.canTypeConnectToType(outlet.getType(), getType());
        if (canTypeConnectToType) { // Wenn outlet auf diesen Type connecten kann
    ...
    public synchronized Outlet getConnectedLet(int index) {
        if(index >= connectedOutlets.size() || index < 0) {
            return null;
        }
