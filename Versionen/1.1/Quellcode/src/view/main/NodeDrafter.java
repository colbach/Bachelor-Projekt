...
import projectrunner.debugger.ContextState;
        ...
        if (node != null) {
            ...
            if (node.getDefinition() instanceof DirectInputNodeDefinition) { // DirectInput-Node
                ...
                if (icon != null) { // Kein else da Zustand innerhalb erstem if aendern kann
                    int centerYIcon = centerY - 10;
                    if (node.getDefinition() instanceof Experimental) {
                        centerYIcon += 5;
                    }

                    icon.drawCentered(g, centerX, centerYIcon);
                } else {
                ...
                if (node.getDefinition() instanceof Experimental) {
                    try {
                        ...
                        experimentalLabelImageAsset.drawCentered(g, centerX, centerY - 27);
