...
public class VirtualLetLister {
    ...
    public static Pair<InOutlet, Integer> getCorrespondingLetForConnectedVirtualLet(InOutlet[] lets, int virtualIndex) {
        int virtualLetCount = 0;
        for (InOutlet let : lets) {
            if (let.isConnected()) {
                if (let.canConnectToMultipleLets()) {
                    for (int i = 0; i < let.getConnectedLetsCount(); i++) {
                        if (virtualIndex == virtualLetCount) { // Virtueller Let
                            return new Pair<>(let, i);
                        }
                        virtualLetCount++;
                    }
                } else {
                    if (virtualIndex == virtualLetCount) { // Let das nur zu einem Let verbinden kann
                        return new Pair<>(let, 0);
                    }
                    virtualLetCount++;
                }
            } else { // Fall: Nicht verbundenes Let
    ...
    public static Pair<InOutlet, Integer> getCorrespondingLetForVirtualLet(InOutlet[] lets, int virtualIndex) {
        int virtualLetCount = 0;
        for (InOutlet let : lets) {
            if (let.isConnected()) {
                if (let.canConnectToMultipleLets()) {
