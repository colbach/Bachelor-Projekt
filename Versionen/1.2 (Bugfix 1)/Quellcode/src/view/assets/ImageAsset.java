...
public class ImageAsset {
    ...
    private ImageAsset(String assetName) throws IOException {
        ...
        assetName = assetName.replace("/", File.separator);
    ...
    public static void preload(ProgressIndicator progressIndicator) {
        ...
        ArrayList<String> listOfFilenamesInAssetPath = new ArrayList();
        if (new File(PATH_FOR_ASSETS).exists()) {
            listOfFilenamesInAssetPath.addAll(Arrays.asList(FileGrabber.getListOfFilenamesInDirectory(PATH_FOR_ASSETS, imageRule)));
            if(new File(PATH_FOR_ASSETS + File.separator + "ui").exists()) {
                for(String s : FileGrabber.getListOfFilenamesInDirectory(PATH_FOR_ASSETS + File.separator + "ui", imageRule)) {
                    listOfFilenamesInAssetPath.add("ui" + File.separator + s);
                }
            }
        }
        String alternativeAssetsDirectory = GeneralSettings.getInstance().getString(GeneralSettings.ALTERNATIV_ASSET_DIRECTORY_KEY);
        if (alternativeAssetsDirectory != null && alternativeAssetsDirectory.length() > 0) {
            if (new File(alternativeAssetsDirectory).exists()) {
                ...
                if(!alternativeAssetsDirectory.endsWith(File.separator)) {
                    alternativeAssetsDirectory = alternativeAssetsDirectory + File.separator;
                }
                if(new File(alternativeAssetsDirectory + "ui").exists()) {
                    for(String s : FileGrabber.getListOfFilenamesInDirectory(alternativeAssetsDirectory + "ui", imageRule)) {
                        listOfFilenamesInAssetPath.add("ui" + File.separator + s);
                    }
                }
            } else {
        ...
        int total = listOfFilenamesInAssetPath.size();
