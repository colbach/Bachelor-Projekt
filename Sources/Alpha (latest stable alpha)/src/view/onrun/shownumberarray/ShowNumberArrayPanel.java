    ...
    protected void paintComponent(Graphics graphics) {
        ...
        if (numbers == null) {
            ...
            for (int i = 0; i < numbersNormalized0To1.length; i++) {
                ...
                if (curve) {
                    g.fillRect(x, /*height+offY*/ offY + height - size, MathHelper.roundUp(steps), size);
                } else {
                    if (lastX != -1 && lastSize != -1) {
                        g.drawLine(lastX + MathHelper.roundUp(steps / 2), height - lastSize + offY, x + MathHelper.roundUp(steps / 2), height - size + offY);
                    }
                }
            ...
            if (mouseX > offX && mouseX < offX + width && mouseY > offY && mouseY < offY + height) {
                ...
                g.drawString("" + MathHelper.roundUp(vX), mouseX + 2, offY + height + 15);
            }
        }
