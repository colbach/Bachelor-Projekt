...
public class PlusNodeDefinition implements NodeDefinition {
    ...
    public String getDescription() {
        return "Addiert A und B." + TAG_PREAMBLE + " [Math] Plus addieren";
    }
    ...
    public void run(InOut io, API api) throws Exception {
        ...
        if (oA instanceof Number && oB instanceof Number) {
            ...
            io.out(0, MatrixAndVectorOperations.add(((Vector) oA), (Vector) oB));

        } else if (oA instanceof Matrix && oB instanceof Matrix) {

            io.out(0, MatrixAndVectorOperations.add(((Matrix) oA), (Matrix) oB));

        } else {
            throw new Exception(oA + " (" + oA.getClass() + ") kann nicht zu " + oB + " (" + oB.getClass() + ") hinzugefuegt werden.");
        }
