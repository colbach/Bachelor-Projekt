...
public class ProjectRepair {
    ...
    public static final boolean CHECK_POSITIONS = true;
    ...
    public static void repairProject(Project project, boolean repair) throws Exception {
        ...
        // Nach Nodes suchen welche zu weit links oder oben positioniert sind...
        if (CHECK_POSITIONS) {
            for (Node node : project.getNodes()) {
                if (node.getUICenterX() < 0) {
                    printChange("Node " + node + " x=" + node.getUICenterX() + " < 0!", "Node " + node + " x->0", repair);
                    if (repair) {
                        node.setUiCenterX(0);
                    }
                }
                if (node.getUICenterY() < 0) {
                    printChange("Node " + node + " y=" + node.getUICenterX() + " < 0!", "Node " + node + " y->0", repair);
                    if (repair) {
                        node.setUiCenterY(0);
                    }
                }
